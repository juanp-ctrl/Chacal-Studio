      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {partners.map((partner, index) => (
            <motion.div
              key={partner.key}
              className="group relative overflow-hidden rounded-2xl bg-primary p-8 transition-all duration-300 hover:shadow-lg border border-transparent hover:border-accent/20 flex flex-col h-full"
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.6, delay: index * 0.1 }}
            >
              <div className="mb-6 relative w-full h-16 flex items-center justify-start">
              ...
              <Text className="text-white/80 leading-relaxed mb-6 font-light flex-grow">
